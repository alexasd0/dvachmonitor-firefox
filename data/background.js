var RequestData = {
	requestData: {},
	Get: function (thread) {
		var nthread = "n" + thread.toString();
		if (this.requestData[nthread] !== undefined)  {
			return this.requestData[nthread];
		} else {
			return "CONNECTION_ERROR";
		}
	},
	
	Set: function (thread, data) {
		var nthread = "n" + thread.toString();
		this.requestData[nthread] = data;
	},
	
	Delete: function (thread) {
		var nthread = "n" + thread.toString();
		if (this.requestData[nthread] !== undefined)  {
			delete this.requestData[nthread];
		}
	}
};

function EmitRequest(Domain, Board, Num) {
	Monitor.log("EmitRequest " +" "+ Domain+" "+Board+" "+Num);
	self.port.emit("to-main", {type: "http-get", domain: Domain, board: Board, thread: Num});
}
...
var MyAlarm = {
	...
	addListener: function (listener) {
		...
		document.addEventListener('alarmevent', listener, false);
	},
	
	removeListener: function (listener) {
		document.removeEventListener('alarmevent', listener, false);
	},
	...
	create: function (Name, Delay) {
		if (this.names.indexOf(Name) > -1)
		{
			...
			this.timers.push(setInterval(function() {
				document.dispatchEvent( new CustomEvent('alarmevent', { 'detail': Name }) );
				
			}, Delay));
			Monitor.log("CREATE alarm");
		}
	}
};
...
var Storage = {
	saveThreads: function() {
		...
		localStorage.threads = JSON.stringify(Threads.getAllAsObjects());
	...
	load: function( func ) {
		...
		if (localStorage.threads)
			Threads.loadFromObjects(JSON.parse(localStorage.threads));
		else
			Threads.loadFromObjects({});
		
		Monitor.log("Threads.getAll "+JSON.stringify(Threads.getAll()));
        func(Threads.getAll());
	}
};
...
function updateCounter() {
    ...
    self.port.emit("to-main", {type: "set-badge", unreads: totalUnreads});
}
...
var Scheduler = {
    ...
    clearListeners: function(num) {
        if(_.isUndefined(this.listeners[num])) {
            ...
            for (var i = 0; i < this.listeners[num].length; i++) {
                ...
                document.removeEventListener('alarmevent', this.listeners[num][i], false);
            }
	...
	unscheduleTask: function (num) {
		...
		MyAlarm.clear(num.toString());
		RequestData.Delete(num);
		Monitor.log("Unscheduling **********" + num);
	},

	scheduleTask: function (threadMap, num, func, delaySecs) {
		...
		var listener = function(event) {
		    if (event.detail == num.toString()) {
				...
				/**
				 * Отправляем реквест в main.js и даем задержку для func(), чтобы запрос успел выполниться
				 * */
				EmitRequest(Settings.domain, thread.board, thread.num);
...
var Updater = {
	...
	getUpdates: function(threadMap) {
        ...
        var thread = threadMap.toObject();
        var resp = RequestData.Get(thread.num);
...
function initListener() {
	self.port.on("to-background", function(request) {
		switch(request.type) {
			case "thread-added":
			...
			case "popup-request":
			    ...
			    self.port.emit("to-popup", {type: "popup-request-re", threads: Threads.getAllAsObjects()});
				break;
			...
			case "popup-markasread":
			    ...
			    if(Threads.has(num)) {
			        ...
			        updateCounter();
			        self.port.emit("to-popup", {type: "popup-markasread-re", threads: Threads.getAllAsObjects()});
			    }
			...
			case "popup-update":
			    ...
			    if(Threads.has(num)) {
			        ...
			        updateCounter();
			        self.port.emit("to-popup", {type: "popup-update-re", threads: Threads.getAllAsObjects()});
			    }
				break;
			...
			case "popup-update-all":
				...
				updateCounter();
				self.port.emit("to-popup", {type: "popup-update-all-re", threads: Threads.getAllAsObjects()});
				break;
				
			case "http-get-re":
				Monitor.log("http-get-re");
				RequestData.Set(request.thread, request.data);
			break;
		}
	});
};
